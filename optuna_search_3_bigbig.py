...
def FullyConnected_params_funnel(trial, final_output_channels):
...
def objective(trial):
    ...
    #ec_params = [(8, (128, 128, 128)), (8, (256, 256, 256)), (8, (512, 512, 512))]
    ec_params = [(10, (128, 128, 128)), (10, (64, 64, 64)), (10, (32, 32, 32))]
    fc_params = [(128, 0.4), (128, 0.4)]

    # Number of epochs
    num_epochs = 30

    # Schedulers
    #lr_scheduler_name = trial.suggest_categorical('lr_scheduler', ['flat+decay', 'one-cycle'])
    lr_scheduler_name = 'flat+decay'

    # Optimizers
    #optimizer_name = trial.suggest_categorical('optimizer', ['adam', 'ranger'])
    ...
    # num_train_samples = 580352 # before reweighting the classes to be balanced
    num_train_samples = 580096
    ...
    if lr_scheduler_name == 'one-cycle': # one-cycle does not use warmup
        ...
        start_lr = trial.suggest_categorical('start_lr', [0.0001, 0.001, 0.01])
